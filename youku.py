...
def parsePage(namelist,html):
    ...
    for i in lis:
        ...
        if html=="":
            continue
        if url[8:12]==str2:
            parsetvinfo(html,i.a.string) #解析url中信息，主持人、地区、类型
        else:
...
def parsetvinfo(html,name):
    ...
    ul=info.find('ul')
    #播放按钮
    button=soup.find('div',class_='p-play')
    print  button
    btn=button.a['href']
    print btn
    #地区
    ...
    fromtv=re.findall('<a .*?>(.*?)</a>',str(alltype.group(0)),re.S)
    for i in fromtv:
        print i
        fromtvdict.append(i)
    #图片
    print str(imginfo)
    print '<img alt="'+name+'" src="(.*?)"/>'
    ...
    urllib.urlretrieve(pic.group(1),'C:/Users/Administrator/Desktop/pic/youku/'+name+'.jpg')



    content=info.find('li',class_='p-row p-intro').text #简介
    update=info.find('li',class_='p-row p-renew').get_text().strip()#最近更新时间

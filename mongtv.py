...
def parsePage(html):
    ...
    for i in lis:
        ...
        html = getHTMLText(url_detail)
        #print url_detail
        if html == "":
            continue
        #print str(imgurl.group(1))
        data=parsetvinfo(html,str(imgurl.group(1)),btn)
        findtvshow(data)
...
def findtvshow(data):
    ...
    if name:
        doc =db.youku.find_one({'update':updatetime})

        if doc:
            if str(doc['update']) == str(updatetime):
                print '该剧目前没有更新xxxxxxxxxxxxxxxx'
            else:
...
def parsetvinfo(html,picurl,btn):
    data={}
    alltypedict = []
    allfrmotvdict=[]
    allacctordict=[]
